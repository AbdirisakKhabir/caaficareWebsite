...
const HospitalDoctors = () => {
    ...
    const fetchHospitalDoctors = async () => {
      ...
      try {
        const response = await axios.get(
          `https://app.caaficare.so/api/doctors/hospital/${hospitalId}`
        );
        
        if (response.data.success) {
          // Check if data is an array (doctors) or object (hospital)
          const responseData = response.data.data;
          let doctorsData = [];
          let hospitalData = null;

          if (Array.isArray(responseData)) {
            // If data is an array, it's the doctors list
            doctorsData = responseData;
            hospitalData = response.data.hospital || null;
          } else if (responseData && typeof responseData === 'object') {
            // If data is an object, it might be the hospital
            // Check if there's a separate doctors field or if doctors are in another field
            if (response.data.doctors && Array.isArray(response.data.doctors)) {
              ...
              // No doctors found, just hospital data
              hospitalData = responseData;
              doctorsData = [];
            }
          }

          // Also check for hospital in response.data.hospital
          ...
          if (doctorsData.length === 0) {
            toast.info("No doctors found for this hospital");
          }
        } else {
          toast.error(response.data.error || "Failed to load doctors");
        }
      } catch (error) {
        console.error("Error fetching hospital doctors:", error);
        toast.error(
          error.response?.data?.error || 
          "Failed to load doctors for this hospital"
        );
      } finally {
        setLoading(false);
      }
  ...
  // Get unique specialties from filtered doctors - add safety check
  const specialties = Array.from(
    new Set(
      Array.isArray(doctors) 
        ? doctors.map((doctor) => doctor.profession).filter(Boolean)
        : []
    )
  ).sort();

  // Filter doctors based on search and specialty - add safety check
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-2 border-[#6CA9F5] mx-auto mb-4"></div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-1">
              {searchTerm || selectedSpecialty
                ...
                : "This hospital doesn't have any doctors listed yet"}
            </p>
          </div>
        ) : (
          <>
